...
@if($articles) 
    ...
    <h3>{{ trans('home.from_blog') }}</h3>
    <div class="recent-post group">
    @foreach($articles as $article)
         <div class="hentry-post group">
            <div class="thumb-img"><img src="{{ asset(env('THEME')) }}/images/articles/{{-- $article->img->mini --}}001-55x55.png" alt="001" title="001" /></div>
            <div class="text">
                <a href="{{ route('articlesShow', ['alias'=>$article->alias]) }}" title="{{ $article->title }}" class="title">{{ $article->title }}</a>
